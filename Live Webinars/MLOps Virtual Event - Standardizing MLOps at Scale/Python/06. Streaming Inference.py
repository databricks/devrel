...
model_uri = f"models://{scope}:{key}@databricks/{model_name}/7"
...
feature_stream = spark.readStream.format("delta").table(feature_table_name).drop("price")
...
predictions_stream = ( feature_stream.withColumn("Prediction", model_udf())
...
( predictions_stream.drop("content")
                    .writeStream
                    .format("delta")
                    .outputMode("append")
